...
try {
    ...
    // MySQL: ON DUPLICATE KEY UPDATE no lugar de ON CONFLICT ... DO UPDATE.
    // VALUES(coluna) reaproveita o valor já enviado (inclusive o BLOB), sem
    // precisar transmitir o arquivo duas vezes no mesmo statement.
    $save = $pdo->prepare(
    ...
    //
    // MySQL não tem RETURNING: fazemos o UPDATE e logo depois o SELECT do
    // status, DENTRO da mesma transação, preservando a atomicidade.
    $promote = $pdo->prepare(
    ...
    $readBack = $pdo->prepare('SELECT status FROM bus_registrations WHERE id = :id LIMIT 1');
    $readBack->execute([':id' => $registrationId]);
    $status = (string) $readBack->fetchColumn();
    $pdo->commit();
